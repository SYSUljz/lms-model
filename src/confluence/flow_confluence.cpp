...
void FlowConfluenceStepOnce(StateParam<T>& state_param_loc, const ConstParam<T>& const_param_loc,
  ...
  auto q = total_runoff_mm * cell_size * cell_size * 0.001 / steps / dx / dt;
  ...
  if (const_param_loc.label == Label::Soil) {
    ...
    state_param_loc.prev_t_flow = SolveSaintVenant(prev_t_flow, q, channel_alpha, channel_beta, dt, dx, upstream_flow_in, prev_t_flow);
